...
public:
    ...
    rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr subVcuNavOdom;
            ...
            ParamServer("radar_sam_imu_preintegration", options)
    {
        ...
        //     "gps_odom", qos, std::bind(&IMUPreintegration::gpsOdometryHander, this, std::placeholders::_1),
        //     odomOpt);
        
        subVcuNavOdom = create_subscription<nav_msgs::msg::Odometry>(
            "vcu_nav_odom", qos,
            std::bind(&IMUPreintegration::vcuNavOdomHandler, this, std::placeholders::_1));
        
        // odomTopic -> odometry/imu
        pubImuOdometry = create_publisher<nav_msgs::msg::Odometry>(odomTopic + "_incremental", qos_imu);

        pubImuPath = create_publisher<nav_msgs::msg::Path>("imu_preint_path", 100);
        ...
        correctionNoise = gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector(6) << 0.5, 0.5, 0.5, 1, 1, 1).finished());   // rad,rad,rad,m, m, m 0.05 0.1
    ...
    {
      std::lock_guard<std::mutex> lock(velMtx);
      currVehicleVel = sqrt(msg->twist.twist.linear.x * msg->twist.twist.linear.x + 
    ...
    {
        //RCLCPP_INFO(get_logger(), "[imuPreintegration::IMUPreintegration]odometryHandler");
        ...
        // static bool firstInvalid = true;
        // static double gpsInvalidTime = -1;
        // if (!isGpsOdomValid(*odomMsg))
        // {
        //     std::cout << "gps odom is not valid. waiting valid gps odom..." << std::endl;
        //     isValid = false;
        //     if (firstInvalid)
        //     {
        //         gpsInvalidTime = currentCorrectionTime;
        //         firstInvalid = false;
        //     }
        //     lastGpsOdom = *odomMsg;
        //     return;
        ...
        //    if (!isValid)
        //    {
        //         std::cout << "gps odom invalid " << currentCorrectionTime - gpsInvalidTime << " s" << std::endl;
        //         isValid = true;
        //         firstInvalid = true;
        //    }
        // }
        ...
        std::cout << r_x << " " << r_y << " " << r_z << " " << r_w << std::endl;
        ...
        if (key == 30)
        {
            ...
            RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "\033[35;1m resetOptimization... \033[0m");
    ...
    if (isValid)
    {
    ...
    #endif 
        // add between odometry factor 貌似意义不大
        ...
        pose_stamped.pose.position.x = prevPose_.translation().x();
        pose_stamped.pose.position.y = prevPose_.translation().y();
        pose_stamped.pose.position.z = prevPose_.translation().z();
        pose_stamped.pose.orientation.x = prevPose_.rotation().toQuaternion().x();
        pose_stamped.pose.orientation.y= prevPose_.rotation().toQuaternion().y();
        pose_stamped.pose.orientation.z = prevPose_.rotation().toQuaternion().z();
        pose_stamped.pose.orientation.w = prevPose_.rotation().toQuaternion().w();
        if (!curr_opt_path.poses.empty())
        {
          double diff_x = prevPose_.translation().x() - curr_opt_path.poses.back().pose.position.x;
          double diff_y = prevPose_.translation().y() - curr_opt_path.poses.back().pose.position.y;
        ...
        if (pubOptPath->get_subscription_count() != 0)
        {
            curr_opt_path.header.stamp = odomMsg->header.stamp;
            curr_opt_path.header.frame_id = odometryFrame;
            pubOptPath->publish(curr_opt_path);
        }

        if (pubOverlayText->get_subscription_count() != 0)
        {
            double curr_vel_x = prevState_.velocity().x();
            double curr_vel_y= prevState_.velocity().y();
            double curr_vel_z = prevState_.velocity().z();
            double curr_vel = sqrt(curr_vel_x * curr_vel_x + curr_vel_y * curr_vel_y + curr_vel_z* curr_vel_z);
    ...
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imu_raw)
    {
        ...
        //cout << "acc_z:" << thisImu.linear_acceleration.z << endl;
        ...
        // 2个imu队列中分别填充imu值
        static bool startUp = true; //做个状态判断
        if (currVehicleVel < 0.1) //车速很低时，不进行添加
        {
            RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "\033[31;1m curr vehicle vel %f < 0.2 \033[0m", currVehicleVel);
            startUp  = false;
            return;
        }
        if (currVehicleVel > 0.1 && !startUp)
        {
            RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "\033[34;1m vehicle start up %f < 0.2 \033[0m", currVehicleVel);
            if (!imuQueOpt.empty())
                imuQueOpt.clear();
            if (!imuQueImu.empty())
                imuQueImu.clear();
...
int main(int argc, char **argv)
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "\033[1;32m----> IMU GPS Optimization Started.\033[0m");
